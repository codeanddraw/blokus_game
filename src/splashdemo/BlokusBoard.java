...
public class BlokusBoard implements Serializable {
    ...
    //rid layout (colour)  
    public static final Color BOARDCOLOR = Color.BLACK;
    public static final Color GRIDLINECOLOR = Color.RED;

    //game rules
    public static final String OFFBOARD = "Oops pieces cannot go out of board. Try again!";
                ...
                if (inBounds) {
                    ...
                    if (gridValue != NONE) {
                        if (value == BlokusPiece.PIECE) {
                            throw new IllegalMoveException(OVERLAP);
                        }
                        if (gridValue == bp.getColor()) {
                            if (value == BlokusPiece.ADJACENT) {
                                throw new IllegalMoveException(ADJACENCY);
                            }
                    ...
                    if (value == BlokusPiece.PIECE) {
                        throw new IllegalMoveException(OFFBOARD);
                    }
                }
            }
        }
        if (!corner) {
            throw new IllegalMoveException(firstMove ? START : CORNER);
        }
